...
from HUB_dictionary.path_dictionary    import Path_DTW_Dictionary, Path_DTW_Dictionary_InterActive
from HUB_dictionary.gesture_dictionary import Gesture_DTW_Dictionary
from HUB_Model.multi_recog import HaarCV_Recognizor, PureScrewDriverRecog, MaunallLable
...
class Recog2Track():
    ...
    def perform_VID_analysis(self,
        ...
        while self.numFrames <= endFrame:
            ...
            cv2.rectangle(NewImg,(0,0),
                          (x,int(0.15*y)),
                          (10,10,10),-1) # Vis Level
            ...
            for modelID, model in enumerate(self.recogModels):
                ...
                if self.trackingFlag[modelID]==0:
                    ...
                    if len(tarBox)>0: # obj been detected
                        ...
                        w = tarBox[2]-tarBox[0]
                        h = tarBox[3]-tarBox[1]
                        tarBox[0] = tarBox[0]-min(int(0.1*w),2)
                        tarBox[1] = tarBox[1]-min(int(0.1*h),2)
                        tarBox[2] = tarBox[2]+min(int(0.1*w),2)
                        tarBox[3] = tarBox[3]+min(int(0.1*h),2)
...
class Recog2TrackIN():
    ...
    def motion_feature_extraction(self, imgAFrecog ,pointsSet,modelID, tarBox_w_h):
        ...
        motionDictionary = Motion_Dictionary()
        PathDictionary = Path_DTW_Dictionary_InterActive()
        if len(pointsSet)>3 :
            last_point = pointsSet[-2]
