...
out_put_talbe = [(a,b)]
def main_test(startFrame, endFrame):
    for i in range(startFrame, endFrame):
        newOb = main(i)
        # init condition => Just Append and assume it is true.
        if len(obervation_seq) < 3:
            obervation_seq.append(newOb)
            real_pridiction = self_estimation(newOb)
        else:
            arg_max_score_function(obervation_seq)
            a = main(newOb)
            obervation_seq.pop(0)
