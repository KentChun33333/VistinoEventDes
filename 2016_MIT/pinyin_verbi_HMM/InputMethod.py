...
class InputMethod(object):
  def __init__(self, lm_file, py_file):
    # 输入法需要用到的参数
    self.lm = self.load_lm(lm_file)
    # {'pinyin':{汉字 : 0.5}}
    self.emission = {}
    # 待求解的拼音输入
    self.pinyins = []
  ...
  def load_lm(self, lm_file):
    """加载二元语言模型"""
    return LanguageModel(lm_file)
...
def main():
  im = InputMethod('RenMinData.txt', 'PinYin.txt')
  print ''
